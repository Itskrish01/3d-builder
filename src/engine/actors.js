...
import { nearestRoadPoint, pickNextRoad, roadById, roadPose, roadSamples, splinePoint } from './roads.js';
import { Q, state } from './state.js';
import { heightAt } from './terrain.js';
import { TAU, clamp, lerp, rnd } from './util.js';
import { World, addObject, ogridMove, recordObjAdd, updateObject } from './world.js';
...
export function pathPose(path, dist) {
...
/* ---- populate ------------------------------------------------------------ */
export function addPeopleToPath(path, count) {
...
export function addVehiclesToRoad(rd, count) {
...
export function populateWorld(density) {
  var added = [], i;
...
/* ---- per-frame update ---------------------------------------------------- */
export var _actorQ = new THREE.Quaternion();
