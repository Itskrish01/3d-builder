...
import { ui } from './host.js';
import { Keys } from './input.js';
import { state } from './state.js';
import { DEG, clamp, hexLin, lerp, mixArr } from './util.js';

/* ==========================================================================
   4. RENDERER + CAMERA
   ========================================================================== */

export var canvas, renderer, scene, camera, cam;   // `cam` = the orbit controller
export var viewW = 1, viewH = 1, basePR = 1;
var viewport;                                      // the element the canvas fills

/**
 * @param {HTMLCanvasElement} canvasEl  owned by React, handed over once
 * @param {HTMLElement} viewportEl      the box the canvas should match
 */
export function initRenderer(canvasEl, viewportEl) {
  canvas = canvasEl;
  viewport = viewportEl || canvasEl.parentElement;
  ...
  camera = new THREE.PerspectiveCamera(state.cam.fov || 52, 1, 0.1, 6000);
  cam = new OrbitCam(camera);

  canvas.addEventListener('webglcontextlost', function (e) {
    e.preventDefault();
    ui.toast('WebGL context lost — reload the page to continue', 'err', 8000);
  });
...
/* Field of view is a camera property rather than a scene one, so it is applied
   here and re-applied after a scene loads. Everything that projects — the brush
   ring, the gizmo, the cursor raycast — reads the same matrix, so they follow. */
export function syncCamera() {
  if (!camera) return;
  var fov = clamp(state.cam.fov || 52, 20, 110);
...
export function resize() {
  ...
  viewW = Math.max(1, viewport.clientWidth);
  viewH = Math.max(1, viewport.clientHeight);
...
/* --------------------------------------------------------------------------
   ...
   -------------------------------------------------------------------------- */
export function OrbitCam(camera) {
  this.cam = camera;
